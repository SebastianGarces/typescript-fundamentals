...
const nonDeclFiles = program.getSourceFiles().filter((sf) => !sf.isDeclarationFile);
 ...
 */
const sfSymbols = nonDeclFiles.map((f) => checker.getSymbolAtLocation(f)).filter(isDefined); // here's the type guard to filter out undefined
